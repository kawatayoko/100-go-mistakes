...
- 5章 文字列
    ...
    - 文字セットとエンコーディング
        ...
        - エンコーディング
            - UTF-8はエンコーディング規格
                UTF-8はすべてのUnicode文字を1〜4byteでエンコーディングできる
            - 文字のリストを2進数に翻訳したもの
    - Unicode
        - コードポイント
    ...
    - 文字列と[]byte
        - ほとんどのプログラマが文字列を好む
        - ほとんどのI/Oは[]byteで行われる
            - io.Reader, io.Writer, io.ReadAllはbyte[]で処理される

- 6章 関数とメソッド
    - インターフェイスは「値」と「型」のペアをいれる箱
        - 「値」と「型」どちらもnilの場合に、nilとみなされる
    - インターフェースはディスパッチラッパー
    - Goの場合メソッドは第1引数をレシーバとする関数のシンタックスシュガー